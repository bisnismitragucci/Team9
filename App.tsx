...
import { Crown, Diamond, Shield, Users, TrendingUp, Menu, X, ChevronDown, ArrowLeft, CheckCircle, MapPin, Info, DollarSign, Image, HelpCircle, Clock, GraduationCap, Headphones, Layers } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleAdminChat = () => {
    ...
    window.open(`https://wa.me/6282124834304?text=${encodeURIComponent(message)}`, "_blank");
  };
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const whatsappUrl = `https://wa.me/6282124834304?text=${encodedMessage}`;
    ...
    <div className="min-h-screen bg-white text-black selection:bg-[#d4af37] selection:text-white font-sans">
      ...
      <nav className={`fixed w-full z-[100] transition-all duration-500 ${scrolled || isMobileMenuOpen ? 'glass-nav py-2 sm:py-3' : 'bg-transparent py-4 sm:py-6'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex justify-between items-center">
          ...
          <div className="hidden lg:flex gap-8 text-[10px] font-bold uppercase tracking-[0.2em] items-center">
            ...
            <button onClick={() => scrollToSection('benefits')} className="hover:text-[#d4af37] transition-all py-2 border-b-2 border-transparent hover:border-[#d4af37] cursor-pointer outline-none">Manfaat</button>
          ...
          <button 
            ...
            className="lg:hidden text-black hover:text-[#d4af37] transition-colors p-2"
          >
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="lg:hidden absolute top-full left-0 w-full bg-white border-b border-[#d4af37]/20 shadow-2xl py-8 px-6 flex flex-col gap-6 text-center"
            >
              <button onClick={() => scrollToSection('about')} className="text-sm font-bold uppercase tracking-widest hover:text-[#d4af37]">Tentang Bisnis</button>
              <button onClick={() => scrollToSection('benefits')} className="text-sm font-bold uppercase tracking-widest hover:text-[#d4af37]">Manfaat</button>
              <button onClick={() => scrollToSection('evaluation')} className="text-sm font-bold uppercase tracking-widest hover:text-[#d4af37]">Evaluasi</button>
              <button onClick={() => scrollToSection('commission')} className="text-sm font-bold uppercase tracking-widest hover:text-[#d4af37]">Komisi</button>
              <button onClick={() => scrollToSection('security')} className="text-sm font-bold uppercase tracking-widest hover:text-[#d4af37]">Keamanan</button>
              <button onClick={() => scrollToSection('faq')} className="text-sm font-bold uppercase tracking-widest hover:text-[#d4af37]">FAQ</button>
              <button onClick={() => scrollToSection('location')} className="text-sm font-bold uppercase tracking-widest hover:text-[#d4af37]">Lokasi</button>
              <button 
                onClick={navigateToRegistration}
                className="bg-gold-gradient text-black py-4 rounded-full font-black uppercase tracking-widest shadow-lg"
              >
                Join Program
              </button>
            </motion.div>
      ...
      <AnimatePresence mode="wait">
        {currentPage === 'home' ? (
          ...
          {/* Benefits Section */}
          <section id="benefits" className="py-20 sm:py-32 px-6 bg-white scroll-mt-24">
            <div className="max-w-7xl mx-auto">
              <div className="text-center mb-16 sm:mb-24">
                <h2 className="text-4xl sm:text-6xl font-serif font-bold mb-6 italic gold-gradient">Manfaat Kemitraan</h2>
                <p className="text-gray-600 text-lg sm:text-xl max-w-2xl mx-auto">Keuntungan eksklusif yang dirancang untuk mendukung kesuksesan finansial Anda bersama Gucci Elite.</p>
              </div>
                ...
                {[
                  {
                    ...
                    desc: "Bimbingan intensif dari Admin Registrasi and Mentor berpengalaman untuk memastikan setiap langkah Anda tepat.",
                    detail: "Kami tidak membiarkan Anda berjalan sendiri. Tim support kami tersedia untuk membantu kendala teknis maupun strategi operasional harian Anda."
                  },
                  {
                    icon: <GraduationCap size={40} />,
                    title: "Pelatihan Eksklusif",
                    desc: "Akses ke modul pelatihan and workshop mingguan untuk meningkatkan efisiensi pengelolaan Akun Kerja.",
                    detail: "Dapatkan wawasan mendalam tentang manajemen aset digital and strategi memaksimalkan komisi melalui program edukasi berkelanjutan kami."
                  },
                  {
                    icon: <Clock size={40} />,
                    title: "Fleksibilitas Waktu",
                    desc: "Kebebasan mengelola waktu kerja Anda sendiri. Hasilkan pendapatan kapan saja and di mana saja.",
                    detail: "Sistem kami berbasis cloud, memungkinkan Anda memantau and mengoperasikan Akun Kerja hanya melalui smartphone dari kenyamanan rumah Anda."
                  },
                  {
                    icon: <TrendingUp size={40} />,
                    title: "Potensi Penghasilan",
                    desc: "Struktur komisi harian yang kompetitif hingga 50% dengan sistem penarikan dana yang instan.",
                    detail: "Nikmati bagi hasil yang transparan. Semakin aktif Anda mengelola akun, semakin besar potensi profit harian yang bisa Anda cairkan."
                  },
                  {
                    icon: <Layers size={40} />,
                    title: "Sumber Daya Eksklusif",
                    desc: "Akses ke dashboard Akun Kerja (E-Wallet) premium dengan fitur monitoring real-time.",
                    detail: "Gunakan infrastruktur teknologi canggih kami untuk melacak setiap transaksi and pertumbuhan saldo Anda secara presisi and aman."
                  },
                  {
                    icon: <Shield size={40} />,
                    title: "Keamanan Terjamin",
                    desc: "Perlindungan data and dana di bawah naungan legalitas PT Graha Citra Prima.",
                    detail: "Kepercayaan adalah aset terpenting. Kami menerapkan protokol keamanan berlapis untuk menjaga privasi and integritas saldo mitra kami."
                  }
                ].map((benefit, idx) => (
                  <motion.div 
                    key={idx}
                    whileHover={{ y: -10 }}
                    onClick={() => setActiveFeature({ title: benefit.title, desc: benefit.detail })}
                    className="luxury-card p-10 rounded-[2.5rem] group cursor-pointer"
                  >
                    ...
                    <div className="text-[10px] text-[#d4af37] font-bold uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-16 text-center md:text-left">
          ...
          <div>
            ...
            <ul className="space-y-6 text-gray-600 text-sm font-bold uppercase tracking-widest">
              ...
              <li><button onClick={() => scrollToSection('benefits')} className="hover:text-[#d4af37] transition-colors cursor-pointer outline-none">Manfaat Kemitraan</button></li>
